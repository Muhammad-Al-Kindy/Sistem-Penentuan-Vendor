...
<?php endif; ?>
                    ...
                    <tbody>
                        <?php $__currentLoopData = $itemsWithPOItemId; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <tr>
                                ...
                                <td class="border border-gray-300 px-4 py-2 text-center"><?php echo e($item->qty_po); ?></td>
                                <td class="border border-gray-300 px-4 py-2">
                                    <input type="number" name="qty_diterima[]" min="0" max="<?php echo e($item->qty_po); ?>"
                                ...
                                <td class="border border-gray-300 px-4 py-2">
                                    ...
                                    <input type="number" name="qty_sesuai[]" min="0" max="<?php echo e($item->qty_po); ?>"
                                        class="w-full border border-gray-300 rounded px-2 py-1"
    ...
    <script>
        ...
        document.querySelector('form[data-update-form]').addEventListener('submit', function(event) {
            const qtyDiterimaInputs = document.querySelectorAll('input[name="qty_diterima[]"]');
            let hasValidItem = false;

            qtyDiterimaInputs.forEach(input => {
        ...
        // Add validation for qty_sesuai not exceeding qty_diterima
        document.addEventListener('input', function(e) {
            if (e.target.name === 'qty_diterima[]' || e.target.name === 'qty_sesuai[]') {
                const row = e.target.closest('tr');
                const qtyDiterima = parseInt(row.querySelector('input[name="qty_diterima[]"]').value) || 0;
                const qtySesuai = parseInt(row.querySelector('input[name="qty_sesuai[]"]').value) || 0;

                if (qtySesuai > qtyDiterima) {
                    row.querySelector('input[name="qty_sesuai[]"]').value = qtyDiterima;
                }
            }
        });
    </script>
