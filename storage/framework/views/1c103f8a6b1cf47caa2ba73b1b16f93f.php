...
<?php endif; ?>
                    ...
                    <table class="w-full table-auto text-sm border border-gray-200 shadow-sm rounded-lg overflow-hidden">
                        ...
                        <thead>
                            <tr class="bg-gray-100 text-gray-700">
                                ...
                                <th class="border px-4 py-2">QTY PO</th>
    ...
    <script>
        document.getElementById('purchase_order_select').addEventListener('change', function() {
            ...
            if (!purchaseOrderId) {
                itemsTableBody.innerHTML =
                    '<tr><td colspan="4" class="text-center text-gray-500 py-4">Pilih perusahaan terlebih dahulu untuk memuat barang.</td></tr>';
                return;
            }
            ...
            fetch(`/purchase-orders/${purchaseOrderId}/items`)
                ...
                .then(data => {
                    if (!Array.isArray(data)) {
                        ...
                        itemsTableBody.innerHTML =
                            '<tr><td colspan="4" class="text-center text-red-500 py-4">Gagal memuat barang.</td></tr>';
                        return;
                    }

                    if (data.length === 0) {
                        itemsTableBody.innerHTML =
                            '<tr><td colspan="4" class="text-center text-gray-500 py-4">Tidak ada barang untuk purchase order ini.</td></tr>';
                        return;
                    }
                    ...
                    data.forEach((item, index) => {
                        ...
                        const qtyOrder = item.kuantitas || 0;
                        ...
                        // QTY Order (display only)
                        const qtyOrderCell = document.createElement('td');
                        qtyOrderCell.className = 'border px-4 py-2 text-center';
                        qtyOrderCell.textContent = qtyOrder;
                        row.appendChild(qtyOrderCell);
                        ...
                        // Hidden input for qty_order
                        const hiddenQtyOrderInput = document.createElement('input');
                        hiddenQtyOrderInput.type = 'hidden';
                        hiddenQtyOrderInput.name = 'qty_order[]';
                        hiddenQtyOrderInput.value = qtyOrder;
                        row.appendChild(hiddenQtyOrderInput);
                        ...
                        qtyDiterimaInput.addEventListener('input', function() {
                        ...
                        qtySesuaiInput.max = qtyOrder;
                        qtySesuaiInput.required = true;
                        qtySesuaiInput.className =
                            'w-full border border-gray-300 rounded-md px-2 py-1 focus:ring-1 focus:ring-blue-500';
                        qtySesuaiInput.addEventListener('input', function() {
                            const val = parseInt(this.value) || 0;
                            if (val > qtyOrder) {
                                this.value = qtyOrder;
                            }
        ...
        document.getElementById('goods_receipt_form').addEventListener('submit', function(event) {
            ...
            rows.forEach(row => {
                const qtyDiterima = row.querySelector('input[name="qty_diterima[]"]');
                if (qtyDiterima && parseInt(qtyDiterima.value) > 0) {
                    hasValidItem = true;
                }
            });
