...
<div id="app"  data-subkriteria-url="{{ route('vendor.index') }}" class="max-w-7xl mx-auto px-4 py-8">
    ...
    <div class="bg-white rounded shadow p-6">
        ...
        <form action="{{ route('vendor.update', $vendor->idVendor) }}" method="POST" class="space-y-6" data-update-form data-redirect-url="{{ route('vendor.index') }}">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="text" name="SPPKP" id="SPPKP"
                       value="{{ old('SPPKP', $vendor->SPPKP) }}"
                       class="w-full border border-gray-300 px-4 py-2 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
            </div>

            <div>
                <label for="nomorIndukPerusahaan" class="block text-sm font-medium text-gray-700 mb-1">Nomor Induk Berusaha Perusahaan</label>
            ...
            <div>
                <label for="jenisPerusahaan" class="block text-sm font-medium text-gray-700 mb-1">Jenis Perusahaan</label>
